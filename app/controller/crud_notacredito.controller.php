<?php
  ...
  switch($accion){
    ...
    case "guardar":
        ...
        try{
          Gestion_NotaCredito::Create($txt_cli_codigo, $txt_nota_numero, $gran_total, $fecha, $_usu_sed_codigo, $txt_facturas, $cuenta_banco);

          $banco = Gestion_Cuentasbanco::Readby($cuenta_banco);
          $tot_saldo = $banco['fin_saldo'] - $gran_total;

          Gestion_NotaCredito::Update_Saldo_Banco($cuenta_banco,$tot_saldo);
          ...
          while ($i <= $index) {
            ...
            $tot           = "total".$i;
            $total         = $_POST[$tot];
            Gestion_NotaCredito::Create_Detalle($producto, $cantidad, $observaciones, $total, $txt_nota_numero);
          $i++;
          }
    ...
    header("location: ../views/default/dashboard.php?m=".base64_encode("module/notas_credito.php")."&pagid=".base64_encode("PAG-100019")."&alert=true&alty=$alert_type&almsn=$alert_msn");
?>
